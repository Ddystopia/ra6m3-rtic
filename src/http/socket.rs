...
use crate::{Mono, Net, poll_share::TokenProvider};

pub struct Timer;
pub struct Socket<R: Mutex<T = Net> + 'static>(pub(super) SocketInner<R>);
pub struct SocketInner<R: 'static>(SocketHandle, TokenProvider<R>);
...
impl<R: Mutex<T = Net>> picoserve::io::Socket for Socket<R> {
    ...
    fn split(&mut self) -> (Self::ReadHalf<'_>, Self::WriteHalf<'_>) {
        (
            Read(self.0.0, self.0.1, PhantomData),
            Write(self.0.0, self.0.1, PhantomData),
        )
    }
    async fn shutdown<Timer: picoserve::Timer>(
        ...
        Ok(shutdown(self.0, timeouts, timer).await)
    }
}
...
impl<R: Mutex<T = Net>> picoserve::io::Read for SocketInner<R> {
    async fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        Read(self.0, self.1, PhantomData).read(buf).await
    }
}

impl<R: Mutex<T = Net>> picoserve::io::Write for SocketInner<R> {
    async fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
        Write(self.0, self.1, PhantomData).write(buf).await
    }
}

// todo: timeout and abort
pub(super) async fn shutdown<R: Mutex<T = Net>, Timer: picoserve::Timer>(
    SocketInner(handle, net): SocketInner<R>,
...
impl<R: Mutex<T = Net>> Drop for Socket<R> {
    fn drop(&mut self) {
        let Socket(SocketInner(handle, net)) = *self;
        net.lock(|net| {
            let socket = net.sockets.get_mut::<smoltcp::socket::tcp::Socket>(handle);
...
impl<R: Mutex<T = Net>> picoserve::io::ErrorType for SocketInner<R> {
    type Error = Error;
}
...
impl<R> Clone for SocketInner<R> {
    fn clone(&self) -> Self {
        *self
    }
}
impl<R> Copy for SocketInner<R> {}
